...
struct GameResult {
     ...
     */
    GameResult(Player winningPlayer, const std::string& endReason, int turnCount, int p1Cells,
               int p2Cells)
        : winner(winningPlayer),
          reason(endReason),
          finalTurnCount(turnCount),
          player1CellCount(p1Cells),
          player2CellCount(p2Cells) {}
     ...
     */
    bool isTie() const { return !winner.has_value(); }
    ...
    std::string toString() const {
        ...
        result += " (" + reason + ")";
        result += " [P1: " + std::to_string(player1CellCount) +
                  ", P2: " + std::to_string(player2CellCount) + "]";
        return result;
    }
};
...
inline const char* winConditionToString(WinCondition condition) {
    switch (condition) {
        case WinCondition::ELIMINATION:
            return "ELIMINATION";
        case WinCondition::TURN_LIMIT_MAJORITY:
            return "TURN_LIMIT_MAJORITY";
        default:
            return "UNKNOWN";
    }
}
...
inline const char* safetyLevelToString(SafetyLevel level) {
    switch (level) {
        case SafetyLevel::VALIDATE_ONLY:
            return "VALIDATE_ONLY";
        case SafetyLevel::LIGHT_UNDO:
            return "LIGHT_UNDO";
        case SafetyLevel::FULL_ROLLBACK:
