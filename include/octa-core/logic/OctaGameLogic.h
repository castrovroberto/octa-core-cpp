...
public:
     ...
     * - No game result yet
     */
    explicit OctaGameLogic(std::shared_ptr<IGameMap> gameMap, const GameConfig& config = GameConfig());
...
private:
     ...
     */
    std::vector<std::shared_ptr<GameCell>> executeChainReaction(std::shared_ptr<GameCell> startCell, Player player);
